...
export class DatabaseInit1644340250838 implements MigrationInterface {
    name = 'DatabaseInit1644340250838'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`order\` (\`id\` int NOT NULL AUTO_INCREMENT, \`type\` enum ('TAKEAWAY', 'DELIVERY') NOT NULL, \`status\` enum ('INACTIVE', 'ACTIVE') NOT NULL, \`opened_at\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`arrival_time\` timestamp NULL, \`restaurant_id\` int NULL, \`user_id\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
