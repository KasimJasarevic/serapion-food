...
export class NotificationService {
  ...
  sendLastCall(restaurant: string) {
    const content = this.createContent('Last call in ' + restaurant + ' order.');
    this.sendNotification(content);
  }
  ...
  private sendNotification(content: any) {
    this._http.post('https://onesignal.com/api/v1/notifications', content, {headers: this.getHeaders()}).subscribe();
  }
